...
namespace Server
{
    public partial class formServer : Form
    {
        ...
        {
            listener = new TcpListener(IPAddress.Parse(txt_IPAddress.Text), Convert.ToInt32(txt_PORT.Text));
        ...
        public void updateUI(String m)
        {
            ...
            {
                txt_chatLog.AppendText(">>" + m + Environment.NewLine);
            });
        }
                ...
                try
                {
                    ...
                    switch (parts[0])
                    {
                        case "gChat":
                            this.Invoke((MethodInvoker)delegate // To Write the Received data
                            {
                                txt_chatLog.Text += username + ": " + parts[1] + Environment.NewLine;
                            });
        ...
        private void Private_Click(object sender, EventArgs e)
        {
            if (listBox1.SelectedIndex != -1)
            {
                ...
                chat.Clear();
                chat.Add("gChat");
                chat.Add("Admin : " + txt_inputPrivate.Text);
        ...
        public void sendUsersList()
        {
            try
            {
                byte[] userList = new byte[Convert.ToInt64(txt_Size.Text)];
        ...
        {
            txt_chatLog.SelectionStart = txt_chatLog.TextLength;
